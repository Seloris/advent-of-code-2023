...
export function step2() {
  const result = data.split('\n').reduce((a, b) => {
    let numbers = b
      .match(new RegExp(/[0-9]|one|two|three|four|five|six|seven|eight|nine/g))!
      .map((x) =>
        Number.isInteger(+x) ? +x : numbersAsLetter.findIndex((n) => n === x)
      );
    const numb =
      a + +(numbers.length == 0 ? 0 : numbers[0] + numbers[numbers.length - 1]);
    console.log({ b, numbers, numb });
    return numb;
  }, 0);
